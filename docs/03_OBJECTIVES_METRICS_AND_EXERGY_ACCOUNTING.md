...
- external ambient heat, reported as `external_heat_input_j_per_kg`: heat-only
  E-303 recovery into the returning coolant;
...
|---|---|---|---|
| LTA-CAES | `heat_offtake=none` | maximum electrical RTE | Rank closed two-tank candidates by expansion work. |
...
product is electricity), while LTAHP's stated product goal is the combined
